...
const size_t MODEL_MEMORY_USAGE = 1024 * 1024 * 1024; // 1GB per instance
...
public:
    ...
    whisper_context* get() const { return ctx; }
...
struct whisper_instance {
    ...
    whisper_instance(int id, std::shared_ptr<WhisperContext> c)
        : id(id), ctx(std::move(c)), in_use(false) {}
};
...
static std::string process_audio(whisper_context* ctx, const whisper_params& params,
    ...
    if (params.response_format == "json") {
        json result = {{"text", whisper_full_get_segment_text(ctx, 0)}};
        return result.dump();
    } else {
...
int main(int argc, char** argv) {
    ...
    int num_worker_threads = num_instances;
    ...
    svr.Post(sparams.request_path + sparams.inference_path,
             [&](const httplib::Request& req, httplib::Response& res) {
                 try {
                     ...
                     auto future_result = task->result_promise.get_future();
                     task_queue.post(task);

                     // Simplify asynchronous handling
                     future_result.wait();
                     res.set_content(future_result.get(), "application/json");
                 } catch (const std::exception& e) {
